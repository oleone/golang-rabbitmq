...
func (r *OrderProductRepositoryMysql) Create(orderProduct *entity.OrderProduct) error {
	_, err := r.DB.Exec("insert into order_products (id, order_id, product_id, quantity) values (?, ?, ?, ?)", orderProduct.ID, orderProduct.OrderID, orderProduct.ProductID, orderProduct.Quantity)
